...
def data2TextDistance(boundedLetter):
	...
	sum=0;
	for itemIndex1 in range(0,len(boundedLetter)):
		for itemIndex2 in range(itemIndex1,len(boundedLetter)):
...
def processData(organizedData):
	...
	while (rotationDegree < 360):
		...
		for letterIndex in range(0,len(letters)):
...
def downloadData():
	...
	javascriptCode = response.content.decode('utf-8')
